...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <van-nav-bar title="私信对话" left-text="返回" left-arrow @click-left="onBack" @click-right="router.push({path: '/index'})">
                    <template #right>
            ...
            <van-pull-refresh v-model="state.isRefreshing" success-text="刷新成功" pull-distance="200" @refresh="onRefresh" style="min-height: 50vh;">
                ...
                <div class="privateMessageChatListModule" >
                    <van-list v-model:loading="state.isLoading" :finished="state.isFinished" v-model:error="state.isError" :offset="10" error-text="请求失败，点击重新加载" direction="up" finished-text="" @load="onLoad" >
            ...
            <div class="privateMessageEnter van-hairline--top">
				<div class="message-container" @click="addPrivateMessageUI();" >
					<div class="messageContent-box">
						...
						<span class="messageContent-text" v-if="form.messageContent == null || form.messageContent == ''">我想说的话...</span>
            ...
            <van-popup v-model:show="form.popup_privateMessage" safe-area-inset-bottom :close-on-popstate="true" position="bottom" closeable close-icon="close" class="custom-bottom-popupModule" :style="{ height: '80%' }" round>
                <div ref="formScrollRef" class="scroll" v-if="form.allowSendPrivateMessage">
                    <van-form ref="formRef" @submit="onSubmit" :scroll-to-error="true" class="form-container" >
                        <van-field v-model="form.messageContent" name="messageContent" placeholder="请输入私信内容" autosize :rows="6" type="textarea" class="messageContent-item" :rules="[{ validator: validatorMessageContent }]"  :error-message="error.messageContent" />
                        ...
                        <div class="submitButton">
                            <van-button round block type="primary" native-type="submit" :disabled="form.allowSubmit">提交</van-button>
...
<script lang="ts" setup>
    ...
    watchEffect(() => {
        ...
        if(titleValue != null && titleValue != ''){
            document.title = '私信对话 - ' + titleValue;
        }
    }, {
