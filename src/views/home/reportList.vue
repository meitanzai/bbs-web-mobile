...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <van-nav-bar title="举报" left-text="返回" left-arrow @click-left="onBack" @click-right="router.push({path: '/index'})">
                    <template #right>
                    ...
                    <van-list v-model:loading="state.isLoading" :finished="state.isFinished" v-model:error="state.isError" error-text="请求失败，点击重新加载" finished-text="没有更多了" @load="onLoad">
                        ...
                        <div v-for="(report,index) in state.reportList" :key="report.id" class="item van-hairline--bottom">
                            ...
                            <div class="middle-container">
                                ...
                                <div class="right-layout">
                                    <van-button plain round type="primary" v-if="report.module == 10" @click="router.push({path: '/thread', query:{ topicId : report.parameterId, reportModule:report.module}})">查看话题</van-button>
                                    <van-button plain round type="primary" v-if="report.module == 20" @click="router.push({path: '/thread', query:{ topicId : report.extraParameterId,commentId:report.parameterId, reportModule:report.module}})">查看评论</van-button>
                                    <van-button plain round type="primary" v-if="report.module == 30" @click="router.push({path: '/thread', query:{ topicId : report.extraParameterId.split('-')[0], commentId:report.extraParameterId.split('-')[1], replyId:report.parameterId, reportModule:report.module}})">查看评论回复</van-button>
                                    <van-button plain round type="primary" v-if="report.module == 40" @click="router.push({path: '/question', query:{ questionId : report.parameterId, reportModule:report.module}})">查看问题</van-button>
                                    <van-button plain round type="primary" v-if="report.module == 50" @click="router.push({path: '/question', query:{ questionId : report.extraParameterId,answerId:report.parameterId, reportModule:report.module}})">查看答案</van-button>
                                    <van-button plain round type="primary" v-if="report.module == 60" @click="router.push({path: '/question', query:{ questionId : report.extraParameterId.split('-')[0], answerId:report.extraParameterId.split('-')[1], replyId:report.parameterId, reportModule:report.module}})">查看答案回复</van-button>
                                </div>
                            </div>
                            <div class="middle-processResult-container" v-if="report.processResult != null && report.processResult != ''">
                                <div class="details"><span class="text">处理结果：</span><span class="val">{{report.processResult}}</span></div>  
                            </div>
                            <div class="bottom-container">
                                <div class="left-layout">
                                        <van-tag type="primary" v-if="report.status ==10">待处理</van-tag>
                                        <van-tag type="warning" v-if="report.status ==40">投拆失败</van-tag>
                                        <van-tag type="success" v-if="report.status ==50">投诉成功</van-tag>
                                </div>
                                <div class="right-layout">
                            ...
                            <div class="middle-lower-container">
                                <div class="details"><span class="text">总金额：</span><span class="val">{{giveRedEnvelope.totalAmount}}元<span class="refundAmount" v-if="parseFloat(giveRedEnvelope.refundAmount) >0"> 返还{{giveRedEnvelope.refundAmount}}</span></span></div>  
                            </div>
                            <div class="bottom-container">
                                <div class="details"><span class="text">发放数量：</span><span class="val">{{giveRedEnvelope.giveQuantity}}</span></div>
                                <div class="details"><span class="text">已领取数量：</span><span class="val">{{giveRedEnvelope.giveQuantity - giveRedEnvelope.remainingQuantity}}</span></div>
                            </div>-->
                        </div>
...
<script lang="ts" setup>
    ...
    watchEffect(() => {
        ...
        if(titleValue != null && titleValue != ''){
            document.title = '举报列表 - ' + titleValue;
        }
    }, {
