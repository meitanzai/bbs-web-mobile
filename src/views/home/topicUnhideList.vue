...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <van-nav-bar title="解锁隐藏标签用户" left-text="返回" left-arrow @click-left="onBack" @click-right="router.push({path: '/index'})">
                    <template #right>
                ...
                <van-list v-model:loading="state.isLoading" :finished="state.isFinished" v-model:error="state.isError" error-text="请求失败，点击重新加载" finished-text="没有更多了" @load="onLoad">
                    ...
                    <div v-for="(topicUnhide,index) in state.topicUnhideList" :key="topicUnhide.id" class="item van-hairline--bottom">
                        <div class="top-container">
                            <div class="userInfo">
                                <div v-if="topicUnhide.account == null || topicUnhide.account == ''" class="cancelAccount">此用户账号已注销</div>
                        ...
                        <div class="bottom-container">
                            <div class="left-layout">
                                <span v-if="topicUnhide.hideTagType == 10" class="tag tag-type-10"><span class="tag-content"><span>输入密码</span></span></span>
                                <span v-if="topicUnhide.hideTagType == 40" class="tag tag-type-40"><span class="tag-content"><span>积分购买</span></span></span>
                                <span v-if="topicUnhide.hideTagType == 50" class="tag tag-type-50"><span class="tag-content"><span>余额购买</span></span></span>
                            </div>
...
<script lang="ts" setup>
    ...
    watchEffect(() => {
        ...
        if(titleValue != null && titleValue != ''){
            document.title = '话题解锁隐藏标签用户 - ' + titleValue;
        }
    }, {
