...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <van-nav-bar title="点赞用户" left-text="返回" left-arrow @click-left="onBack" @click-right="router.push({path: '/index'})">
                    <template #right>
                ...
                <van-list v-model:loading="state.isLoading" :finished="state.isFinished" v-model:error="state.isError" error-text="请求失败，点击重新加载" finished-text="没有更多了" @load="onLoad">
                    ...
                    <div v-for="(like,index) in state.likeList" :key="like.id" class="item van-hairline--bottom">
                        <div class="top-container">
                            <div class="userInfo">
                                <div v-if="like.account == null || like.account == ''" class="cancelAccount">此用户账号已注销</div>
...
<script lang="ts" setup>
    ...
    watchEffect(() => {
        ...
        if(titleValue != null && titleValue != ''){
            document.title = '点赞用户 - ' + titleValue;
        }
    }, {
