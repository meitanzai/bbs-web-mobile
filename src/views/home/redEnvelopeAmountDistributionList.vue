...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <van-nav-bar title="金额分配" left-text="返回" left-arrow @click-left="onBack" @click-right="router.push({path: '/index'})">
                    <template #right>
                    ...
                    <div class="giveRedEnvelopeInfo">
                        <div class="top-container">
                            <div class="left-layout">
                                <span class="tag" v-if="state.giveRedEnvelope.type == 10">个人定向红包</span>
                                <span class="tag" v-if="state.giveRedEnvelope.type == 20">公共随机红包</span>
                                <span class="tag" v-if="state.giveRedEnvelope.type == 30">公共定额红包</span>
                            </div>
                        ...
                        <div class="middle-lower-container">
                            <div class="details"><span class="text">总金额：</span><span class="val">{{state.giveRedEnvelope.totalAmount}}元<span class="refundAmount" v-if="parseFloat(state.giveRedEnvelope.refundAmount) >0"> 返还{{state.giveRedEnvelope.refundAmount}}</span></span></div>  
                        </div>
                        <div class="bottom-container">
                            <div class="details"><span class="text">发放数量：</span><span class="val">{{state.giveRedEnvelope.giveQuantity}}</span></div>
                            <div class="details"><span class="text">已领取数量：</span><span class="val">{{state.giveRedEnvelope.giveQuantity - state.giveRedEnvelope.remainingQuantity}}</span></div>
                        </div>
                    </div>
                </div>
                        ...
                        <div v-for="(receiveRedEnvelope,index) in state.receiveRedEnvelopeList" :key="receiveRedEnvelope.id" class="item van-hairline--bottom">
                            <div class="top-container">
                                <div class="userInfo">
                                    ...
                                    <div v-if="receiveRedEnvelope.receiveAccount == null || receiveRedEnvelope.receiveAccount == ''" class="cancelAccount">此用户账号已注销</div>
                                </div>
...
<script lang="ts" setup>
    ...
    watchEffect(() => {
        ...
        if(titleValue != null && titleValue != ''){
            document.title = '发红包金额分配列表 - ' + titleValue;
        }
    }, {
