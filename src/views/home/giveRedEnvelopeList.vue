...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <van-nav-bar title="发红包列表" left-text="返回" left-arrow @click-left="onBack" @click-right="router.push({path: '/index'})">
                    <template #right>
                    ...
                    <div v-for="(giveRedEnvelope,index) in state.giveRedEnvelopeList" :key="giveRedEnvelope.id" class="item van-hairline--bottom">
                        <div class="top-container">
                            <div class="left-layout">
                                <span class="tag" v-if="giveRedEnvelope.type == 10">个人定向红包</span>
                                <span class="tag" v-if="giveRedEnvelope.type == 20">公共随机红包</span>
                                <span class="tag" v-if="giveRedEnvelope.type == 30">公共定额红包</span>
                            </div>
                        ...
                        <div class="middle-container">
                            ...
                            <div class="right-layout">
                                <van-button plain round type="primary" @click="router.push({path: '/user/control/redEnvelopeAmountDistributionList', query:{ giveRedEnvelopeId : giveRedEnvelope.id}})">金额分配</van-button>
                            </div>
                        </div>
...
<script lang="ts" setup>
    ...
    watchEffect(() => {
        ...
        if(titleValue != null && titleValue != ''){
            document.title = '发红包列表 - ' + titleValue;
        }
    }, {
