...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <van-nav-bar title="登录日志" left-text="返回" left-arrow @click-left="onBack" @click-right="router.push({path: '/index'})">
                    <template #right>
                            ...
                            <div class="top-container">
                                ...
                                <div class="right-layout">
                                    <span class="state" :class="userLoginLog.typeNumber == 10 ? 'login' : 'renewal'">
                                        <span v-if="userLoginLog.typeNumber == 10">登录</span>
                                        <span v-if="userLoginLog.typeNumber == 20">续期</span>
                                    </span>
                                </div>
                            </div>
                        </div>
...
<script lang="ts" setup>
    ...
    watchEffect(() => {
        ...
        if(titleValue != null && titleValue != ''){
            document.title = '用户登录日志列表 - ' + titleValue;
        }
    }, {
