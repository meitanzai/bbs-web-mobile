...
<template>
     <div class="main">
        <div class="main-container">
            <van-sticky>
                <van-nav-bar title="付款" left-text="返回" left-arrow @click-left="onBack" @click-right="router.push({path: '/index'})">
                    <template #right>
...
<script lang="ts" setup>
    ...
    const validatorRechargeAmount = (val:any):Promise<boolean | string> => {
        return new Promise((resolve) => {
            if(val != ''){
                if(val.length >12){
                    error.rechargeAmount = "不能超过12位数字"
                    resolve(false);
                } else { 
                    proxy?.$axios({
