...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <van-nav-bar title="我的问题" left-text="返回" left-arrow @click-left="onBack" @click-right="router.push({path: '/index'})">
                    <template #right>
...
<script lang="ts" setup>
    ...
    watchEffect(() => {
        ...
        if(titleValue != null && titleValue != ''){
            document.title = '我的问题 - ' + titleValue;
        }
    }, {
    ...
    const onPopoverActions = (question:Question) => {
        ...
        actions.push({ text: '收藏用户', questionId:question.id, className:'custom-popoverModule-1'});
        actions.push({ text: '追加', questionId:question.id, className:'custom-popoverModule-1'});
        return actions;
    }
    ...
    const onPopoverSelect = (action:any) => {
        ...
        }else if(action.text == '追加'){
            router.push({path: '/user/appendQuestion', query:{ questionId : action.questionId}})
        }
    }
