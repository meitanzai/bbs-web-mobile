...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <van-nav-bar title="我的答案" left-text="返回" left-arrow @click-left="onBack" @click-right="router.push({path: '/index'})">
                    <template #right>
                ...
                <van-list v-model:loading="state.isLoading" :finished="state.isFinished" v-model:error="state.isError" error-text="请求失败，点击重新加载" finished-text="没有更多了" @load="onLoad">
                    ...
                    <div v-for="(answer,index) in state.answerList" :key="answer.id" class="item van-hairline--bottom">
                        ...
                        <div class="bottom-container">
                            <div class="layout">
                                <span class="status" v-if="answer.status == 10">[待审核]</span>
...
<script lang="ts" setup>
    ...
    watchEffect(() => {
        ...
        if(titleValue != null && titleValue != ''){
            document.title = '我的答案 - ' + titleValue;
        }
    }, {
