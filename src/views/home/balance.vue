...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <van-nav-bar title="余额" left-text="返回" left-arrow @click-left="onBack" @click-right="router.push({path: '/index'})">
                    <template #right>
                ...
                <div class="headModule">
                    <div class="container">
                        ...
                        <div class="right-layout">
                            <van-button plain round type="primary" @click="router.push({path: '/user/control/payment', query:{ paymentModule : 5}})">充值</van-button>
                        </div>
                    </div>
                </div>
                    ...
                    <van-list v-model:loading="state.isLoading" :finished="state.isFinished" v-model:error="state.isError" error-text="请求失败，点击重新加载" finished-text="没有更多了" @load="onLoad">
                        ...
                        <div v-for="(paymentLog,index) in state.paymentLogList" :key="paymentLog.paymentRunningNumber" class="item van-hairline--bottom">
                            <div class="top-container">
                                <div class="layout">
                                    <div class="runningNumber">支付流水号：{{paymentLog.paymentRunningNumber}}</div>
                                </div>
                            </div>
                            <div class="middle-container">
                                <div class="layout">
                                    <div :class="paymentLog.amountState == 1 ? 'income' : 'outgoings'">
                                        <p v-if="paymentLog.amountState == 1">+{{paymentLog.amount}}元</p>
                                        <p v-if="paymentLog.amountState == 2">-{{paymentLog.amount}}元</p>
                            ...
                            <div class="bottom-container">
                                <div class="left-layout">
                                    <span class="module" v-if="paymentLog.paymentModule == 1">会员卡订单支付</span>
                                    <span class="module" v-if="paymentLog.paymentModule == 5">充值</span>
                                    <span class="module" v-if="paymentLog.paymentModule == 70">余额购买话题隐藏内容</span>
                                    <span class="module" v-if="paymentLog.paymentModule == 80">解锁话题隐藏内容分成</span>
                                    <span class="module" v-if="paymentLog.paymentModule == 90">悬赏金额</span>
                                    <span class="module" v-if="paymentLog.paymentModule == 100">采纳答案</span>
                                    <span class="module" v-if="paymentLog.paymentModule == 110">调整赏金</span>
                                    <span class="module" v-if="paymentLog.paymentModule == 120">话题发红包</span>
                                    <span class="module" v-if="paymentLog.paymentModule == 130">话题收红包</span>
                                    <span class="module" v-if="paymentLog.paymentModule == 140">话题返还红包</span>
                                </div>
                                <div class="right-layout">
                                    <span class="time">
                                        {{paymentLog.times}}
