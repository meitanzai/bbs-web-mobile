...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <van-nav-bar title="我的话题" left-text="返回" left-arrow @click-left="onBack" @click-right="router.push({path: '/index'})">
                    <template #right>
...
<script lang="ts" setup>
    ...
    watchEffect(() => {
        ...
        if(titleValue != null && titleValue != ''){
            document.title = '我的话题 - ' + titleValue;
        }
    }, {
    ...
    const onPopoverActions = (topic:Topic) => {
      //  [{ text: '解锁隐藏标签用户', topicId:topic.id, className:'custom-popoverModule-1'},{ text: '收藏用户', topicId:topic.id, className:'custom-popoverModule-1'},{ text: '点赞用户', topicId:topic.id, className:'custom-popoverModule-1'}]
        ...
        actions.push({ text: '解锁隐藏标签用户', topicId:topic.id, className:'custom-popoverModule-1'});
        actions.push({ text: '收藏用户', topicId:topic.id, className:'custom-popoverModule-1'});
        actions.push({ text: '点赞用户', topicId:topic.id, className:'custom-popoverModule-1'});

        if(topic.giveRedEnvelopeId != null && topic.giveRedEnvelopeId != ''){
            actions.push({ text: '红包', giveRedEnvelopeId:topic.giveRedEnvelopeId, className:'custom-popoverModule-1'});
        }
        actions.push({ text: '编辑', topicId:topic.id, className:'custom-popoverModule-1'});
        return actions;
    }
