...
<template>
    <div class="main">
        <div class="main-container">
            <van-sticky>
                <van-nav-bar title="点赞" left-text="返回" left-arrow @click-left="onBack" @click-right="router.push({path: '/index'})">
                    <template #right>
                ...
                <van-list v-model:loading="state.isLoading" :finished="state.isFinished" v-model:error="state.isError" error-text="请求失败，点击重新加载" finished-text="没有更多了" @load="onLoad">
                    ...
                    <div v-for="(like,index) in state.likeList" :key="like.id" class="item van-hairline--bottom">
                        ...
                        <div class="bottom-container">
                            <router-link tag="div" class="content" :to="{path: '/thread', query:{ topicId : like.topicId}}">
                                {{like.topicTitle}}
                            </router-link>
                        </div>
                    </div>
